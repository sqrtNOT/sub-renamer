...
folderpath=input("Enter the directory that contains both the subs and video files (default current directory):") or "."
...
for i in os.listdir(folderpath):
	if(re.search(f"{subext}$",i)!=None):
		subfiles.append(i)
	if(re.search(f"{videoext}$",i)!=None):
		videofiles.append(i)
...
for sub,vid in zip(subfiles,videofiles):
	...
	os.rename(
		os.path.join(folderpath,sub),
		os.path.join(folderpath,destination))
